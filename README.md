...
Лендинг персонального планера Tempo. Портфолийна робота, статичний сайт
без бекенду.

Наразі реалізовано: hero-секція (десктоп + окрема мобільна композиція).
Інші секції — окремими задачами.
...
Базовий рантайм Next.js 15 App Router (React 19 + клієнтський роутинг)
важить ~103 КБ gzip ще до жодного власного рядка коду — це вже перевищує
цільові 100 КБ на перший екран. Власний код hero (Motion + Lenis +
компоненти) додає ще ~45 КБ gzip зверху. Це структурне обмеження вибору
App Router, а не результат цієї реалізації.
